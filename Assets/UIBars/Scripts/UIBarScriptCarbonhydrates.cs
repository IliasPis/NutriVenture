...
public class UIBarScriptCarbonhydrates : MonoBehaviour {
    ...
    public float NewValue = 32.5f; // Default value set to 32.5
    ...
    void Update()
    {
        ...
        Value = Mathf.Clamp(Value, 0f, 65f);

        // Move the Filler position to display the Correct Percent
        RectTransform FRT = (Filler.transform as RectTransform);
        FRT.position = Vector3.Lerp(Mask0, Mask1, Value / 65f);

        // Set the color for the Fill Image, and the Text Objects
        Filler.GetComponent<Image>().color = HPColor.Evaluate(Value / 65f);
        PercentTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 65f);
        RatioTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 65f);
        ...
        foreach (CriteriaRule CR in CriteriaRules)
        {
            if (CR.isImage())
            {
                CR.DefaultColor = HPColor.Evaluate(Value / 65f);
            }
            else
            {
                CR.DefaultColor = TextColor.Evaluate(Value / 65f);
            }
    ...
    public void UpdateValue(float HP, float MaxHP)
    {
        ...
        // NewValue
        NewValue = NewValue + Mathf.Round((HP / MaxHP) * 65f);
