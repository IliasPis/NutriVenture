...
public class UIBarScriptCalories : MonoBehaviour {
    ...
    public float NewValue = 285f; // Default value set to 285
    ...
    void Update()
    {
        ...
        Value = Mathf.Clamp(Value, 0f, 630f);

        // Move the Filler position to display the Correct Percent
        RectTransform FRT = (Filler.transform as RectTransform);
        FRT.position = Vector3.Lerp(Mask0, Mask1, Value / 630f);

        // Set the color for the Fill Image, and the Text Objects
        Filler.GetComponent<Image>().color = HPColor.Evaluate(Value / 630f);
        PercentTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 630f);
        RatioTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 630f);
        ...
        foreach (CriteriaRule CR in CriteriaRules)
        {
            if (CR.isImage())
            {
                CR.DefaultColor = HPColor.Evaluate(Value / 630f);
            }
            else
            {
                CR.DefaultColor = TextColor.Evaluate(Value / 630f);
            }
    ...
    public void UpdateValue(float HP, float MaxHP)
    {
        ...
        // NewValue
        NewValue += NewValue + Mathf.Round((HP / MaxHP) * 630f);
