...
public class UIBarScript : MonoBehaviour {
    ...
    public float NewValue = 0f; // Default value set to 12.5
    ...
    void Update()
    {
        ...
        RectTransform FRT = (Filler.transform as RectTransform);
        FRT.position = Vector3.Lerp(Mask0, Mask1, Value / 25f);

        // Set the color for the Fill Image, and the Text Objects
        Filler.GetComponent<Image>().color = HPColor.Evaluate(Value / 25f);
        PercentTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 25f);
        RatioTxt.GetComponent<Text>().color = TextColor.Evaluate(Value / 25f);
        ...
        foreach (CriteriaRule CR in CriteriaRules)
        {
            if (CR.isImage())
            {
                CR.DefaultColor = HPColor.Evaluate(Value / 25f);
            }
            else
            {
                CR.DefaultColor = TextColor.Evaluate(Value / 25f);
            }
    ...
    public void UpdateValue(float HP, float MaxHP)
    {
        ...
        NewValue += Mathf.Round((HP / MaxHP) * 25f);
